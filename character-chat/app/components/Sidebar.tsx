...
export default function Sidebar({ recentCharacters: initialRecents = [] }: SidebarProps) {
  ...
  const fetchSubscription = async (userId: string) => {
    try {
      // We can use the API we built earlier or a direct check
      const res = await fetch('/api/user/subscription', {
        headers: { 'x-user-id': userId }
      });
      if (res.ok) {
        const data = await res.json();
        setPlanId(data.status || 'free');
      }
    } catch (err) {
  ...
  const getPlanLabel = (p: string) => {
    switch (p) {
      case 'pro': return 'AGENTWOOD PRO';
      case 'starter': return 'AGENTWOOD +';
      default: return 'FREE PLAN';
    }
  };

  const SidebarLink = ({ active, icon, label, badge, onClick, href }: any) => {
    const content = (
      <button
        onClick={onClick}
  ...
  return (
    <aside className="fixed left-0 top-0 hidden h-full w-60 flex-col border-r lg:flex z-50 border-white/5 bg-[#0f0f0f] p-5">
      <div className="mb-8 flex items-center gap-2 px-1 cursor-pointer group" onClick={() => router.push('/home')}>
      ...
      <Link href="/create" className="mb-6 flex w-full items-center justify-center gap-2 rounded-full bg-white py-2.5 text-xs font-black text-black transition-all hover:bg-gray-200 active:scale-95 shadow-lg">
      ...
      <nav className="flex flex-col gap-5 overflow-y-auto scrollbar-hide flex-1">
        <section>
          ...
          <div className="flex flex-col gap-0.5">
            <SidebarLink icon={<Compass size={16} />} label="Discover" href="/home" />
            ...
            <SidebarLink icon={<Book size={16} />} label="Blog" href="/blog" />
          </div>
        </section>
        ...
        <section>
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <SidebarLink icon={<Award size={16} />} label="Rewards" badge="PRO" href="/rewards" />
          </div>
        </section>
      ...
      <div className="mt-auto flex flex-col gap-3 border-t border-white/5 pt-5">
        <SidebarLink icon={<Book size={16} />} label="About" href="/about" />
        ...
        {planId !== 'pro' && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              {planId === 'free' ? 'Unlock Agentwood+' : 'Upgrade to Pro'}
            </div>
        ...
        <div className="relative">
          <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center justify-between w-full gap-2.5 rounded-2xl p-2.5 bg-white/5 border border-white/5 hover:bg-white/10 transition-colors cursor-pointer">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0 text-left">
                ...
                <p className="text-[8px] opacity-40 font-medium uppercase tracking-tighter">{isLoggedIn ? getPlanLabel(planId) : 'Not logged in'}</p>
              </div>
            </div>
          ...
          {showUserMenu && (
            <div className="absolute bottom-full left-0 right-0 mb-2 bg-[#1a1a1a] border border-white/10 rounded-xl overflow-hidden shadow-lg z-50">
              {isLoggedIn ? (
                <>
                  <Link href="/settings" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-3 text-sm text-white/80 hover:bg-white/10 transition-colors">
                    <User size={16} /> My Account
                  </Link>
                  <button onClick={async () => { setShowUserMenu(false); await supabase?.auth.signOut(); router.push('/'); router.refresh(); }} className="flex items-center gap-3 w-full px-4 py-3 text-sm text-red-400 hover:bg-white/10 transition-colors border-t border-white/5">
                    <LogOut size={16} /> Log out
                  </button>
                </>
              ) : (
                <Link href="/login" onClick={() => setShowUserMenu(false)} className="flex items-center gap-3 px-4 py-3 text-sm text-white/80 hover:bg-white/10 transition-colors">
                  <User size={16} /> Log in
                </Link>
              )}
            </div>
