...
interface PageProps {
  params: Promise<{ category: string; archetype: string }>;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateSEOMetadata({
    ...
    keywords: [
      `${decodedArchetype} ${decodedCategory}`,
      `${decodedCategory} ${decodedArchetype} characters`,
      'AI characters',
      'character chat',
    ],
    url: `/category/${category}/archetype/${archetype}`,
  });
}
...
export default async function CategoryArchetypePage({ params, searchParams }: PageProps) {
  ...
  const baseUrl = `/category/${category}/archetype/${archetype}`;
          ...
          <section className="bg-gradient-to-br from-indigo-600 via-purple-700 to-pink-600 text-white py-16 px-6">
            <div className="max-w-7xl mx-auto">
              <h1 className="text-4xl md:text-5xl font-bold mb-4">
                {decodedArchetype} {decodedCategory} AI Characters
              </h1>
              <p className="text-xl text-indigo-100 max-w-2xl">
                Discover {totalCount.toLocaleString()} {decodedArchetype.toLowerCase()} {decodedCategory.toLowerCase()} characters
              </p>
            </div>
          </section>
