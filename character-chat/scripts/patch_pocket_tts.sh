...
# Target file
TARGET="/opt/pocket-tts-primary/lib/python3.12/site-packages/pocket_tts/main.py"
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 LIFESPAN: Checking model initialization...")
    if tts_model is None:
        logger.info(f"⚙️ LIFESPAN: Loading model {DEFAULT_VARIANT}...")
        tts_model = TTSModel.load_model(DEFAULT_VARIANT)
        
        logger.info(f"⚙️ LIFESPAN: Loading default voice state {DEFAULT_AUDIO_PROMPT}...")
        global_model_state = tts_model.get_state_for_audio_prompt(DEFAULT_AUDIO_PROMPT)
        
        state_size = size_of_dict(global_model_state) // 1e6
        logger.info(f"✅ LIFESPAN: Model loaded successfully. State size: {state_size} MB")
    else:
        logger.info("ℹ️ LIFESPAN: Model already initialized.")
    yield
...
def serve(
    ...
    # Note: Logic moved to lifespan to support uvicorn string loading
...
echo "🚀 Restarting service to apply patch..."
systemctl restart pocket-tts
echo "✅ Patch applied. Waiting for server..."
sleep 5
