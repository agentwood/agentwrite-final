...
import { Feather, ChevronDown, ArrowRight, Sparkles, Menu, X, Layout, Users, CreditCard, LogOut } from 'lucide-react';
...
const Navigation = () => {
  ...
  useEffect(() => {
    // Check for user session (Simulated or Real)
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-lg border-b border-stone-100 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 h-16 md:h-20 flex items-center justify-between relative">
        ...
        <div className="hidden md:flex items-center gap-1">
            ...
            <div className="px-4 py-6 cursor-pointer text-sm font-medium text-slate-600 hover:text-slate-900 transition">Business</div>
            ...
            <div className="px-4 py-6 cursor-pointer text-sm font-medium text-slate-600 hover:text-slate-900 transition">About</div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
            {isLoggedIn ? (
                <button 
                  ...
                  className="bg-slate-900 text-white px-5 py-2.5 rounded-lg font-medium hover:bg-slate-800 transition shadow-lg shadow-slate-200 hover:shadow-xl text-sm flex items-center gap-2"
                >
                  <Layout size={16} /> Go to Studio
                </button>
            ) : (
                <>
                    <button 
          ...
          <div className="md:hidden absolute top-16 left-0 right-0 bg-white border-b border-stone-200 shadow-2xl p-6 flex flex-col gap-2 z-40 animate-fade-in-down max-h-[85vh] overflow-y-auto">
              ...
              {isLoggedIn ? (
                  <button 
                    ...
                    className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold mt-2 flex items-center justify-center gap-2"
                  >
                      <Layout size={18} /> Resume Writing
                  </button>
              ) : (
                  <button 
