...
const DashboardPage = () => {
    ...
    // TABS: 'studio' (Standard) | 'aicreate' (Interactive) | 'agentwrite' (Long-Form)
    const [activeTab, setActiveTab] = useState<'studio' | 'aicreate' | 'agentwrite'>('studio');
    ...
    // Modal State
    const [activeModal, setActiveModal] = useState<'none' | 'share' | 'media' | 'new_agent_project'>('none');
    const [selectedProject, setSelectedProject] = useState<any>(null);
    const [mediaType, setMediaType] = useState<'Audio' | 'Video'>('Audio');

    // New Project State
    const [newProjectTitle, setNewProjectTitle] = useState('');
    const [newProjectGenre, setNewProjectGenre] = useState('Non-Fiction');
    const [newProjectWordCount, setNewProjectWordCount] = useState(2000);
    ...
    const agentProjects = [
        { id: '1', title: 'The Future of AI Agents', genre: 'Technical Article', targetWordCount: 2000, status: 'draft', updatedAt: new Date().toISOString() }
    ];
    ...
    const handleCreateAgentProject = () => {
        ...
        // For now, just navigate to the editor with a new ID
    ...
    return (
        <div className="min-h-screen bg-stone-50 font-sans relative pb-12">
            ...
            {/* New Agent Project Modal */}
            {activeModal === 'new_agent_project' && (
                <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-xl shadow-2xl z-50 overflow-hidden border border-stone-200 animate-fade-in-up m-4">
                    <div className="p-6 border-b border-stone-100 flex justify-between items-center bg-stone-50">
                        <h3 className="font-serif text-xl text-slate-900">New Long-Form Project</h3>
                    ...
                    <div className="p-6 space-y-4">
                        <div>
                            ...
                            <input
                                type="text"
                                className="w-full p-3 rounded-lg border border-stone-200 focus:border-indigo-500 outline-none"
                        ...
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Genre / Type</label>
                            <select
                                className="w-full p-3 rounded-lg border border-stone-200 focus:border-indigo-500 outline-none"
                            ...
                            >
                                ...
                                <option>Novel Chapter</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Target Word Count</label>
                            <input
                                type="number"
                                className="w-full p-3 rounded-lg border border-stone-200 focus:border-indigo-500 outline-none"
                                value={newProjectWordCount}
                                onChange={(e) => setNewProjectWordCount(Number(e.target.value))}
                                step={500}
                                min={500}
                            />
                        </div>
                        <div className="pt-4">
                            <button
                                onClick={handleCreateAgentProject}
            ...
            <header className="bg-white border-b border-stone-200 sticky top-0 z-20">
                ...
                <div className="max-w-7xl mx-auto px-6 flex gap-8 border-t border-stone-50 overflow-x-auto no-scrollbar">
                    ...
                    <button
                        onClick={() => setActiveTab('agentwrite')}
                        className={`py-3 text-sm font-bold border-b-2 transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'agentwrite' ? 'text-indigo-600 border-indigo-600' : 'text-slate-400 border-transparent hover:text-indigo-500'}`}
                    >
                        <Bot size={14} /> AgentWrite (Long-Form)
                    </button>
                    <button
                        onClick={() => setActiveTab('aicreate')}
                        className={`py-3 text-sm font-bold border-b-2 transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'aicreate' ? 'text-purple-600 border-purple-600' : 'text-slate-400 border-transparent hover:text-purple-500'}`}
                    >
            ...
            <main className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
                ...
                {activeTab === 'studio' && (
                    <div className="flex flex-col lg:flex-row gap-10 items-start animate-fade-in">
                        <div className="flex-1 w-full">
                            ...
                            {/* Promo Card for AgentWrite */}
                            <div onClick={() => setActiveTab('agentwrite')} className="bg-white border border-stone-200 rounded-xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between cursor-pointer hover:border-indigo-300 hover:shadow-md transition group relative overflow-hidden gap-4">
                                ...
                                <div className="relative z-10 flex items-center gap-4 w-full md:w-auto">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-900">Try AgentWrite</h3>
                                        <p className="text-slate-500 text-sm">Plan and write long-form content with AI agents.</p>
                                    </div>
                                </div>
                ...
                {/* VIEW: AGENTWRITE DASHBOARD */}
                {activeTab === 'agentwrite' && (
                    <div className="animate-fade-in">
                        {/* Hero Banner */}
                        <div className="bg-gradient-to-br from-indigo-900 to-purple-900 rounded-2xl p-6 md:p-10 text-white mb-10 relative overflow-hidden shadow-2xl">
                            <div className="absolute top-0 right-0 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl -mr-20 -mt-20"></div>
                            <div className="relative z-10 max-w-2xl">
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-indigo-100 text-xs font-bold uppercase tracking-wider mb-6">
                                    <Bot size={12} /> AgentWrite V1.0
                                </div>
                                <h2 className="font-serif text-3xl md:text-5xl mb-4 leading-tight">Orchestrate your <br /><span className="text-indigo-300 italic">AI Writing Team.</span></h2>
                                <p className="text-indigo-100 text-base md:text-lg mb-8 leading-relaxed">
                                    Plan, write, and refine long-form content with specialized AI agents.
                                    From detailed outlines to polished prose, let your agents do the heavy lifting.
                                </p>
                                <button
                                    onClick={() => setActiveModal('new_agent_project')}
                                    className="w-full md:w-auto bg-white text-indigo-900 px-8 py-4 rounded-xl font-bold text-lg hover:bg-indigo-50 transition shadow-lg flex items-center justify-center gap-2 group"
                                >
                                    New Long-Form Project <PenTool size={20} className="group-hover:scale-110 transition-transform" />
                                </button>
                            </div>
                        </div>

                        {/* Agent Projects List */}
                        <h3 className="font-serif text-xl text-slate-900 mb-4">Your Agent Projects</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="border-2 border-dashed border-stone-200 rounded-xl p-8 flex flex-col items-center justify-center text-center text-slate-400 hover:border-indigo-300 hover:bg-indigo-50/10 transition cursor-pointer group" onClick={() => setActiveModal('new_agent_project')}>
                                <div className="w-12 h-12 bg-stone-100 rounded-full flex items-center justify-center mb-4 group-hover:bg-white group-hover:shadow-sm transition">
                                    <Plus size={24} className="opacity-50 group-hover:opacity-100 text-indigo-500" />
                                </div>
                                <span className="font-bold text-sm text-slate-600">Create New Project</span>
                            </div>

                            {agentProjects.map((project) => (
                                <div key={project.id} onClick={() => navigate(`/project/${project.id}`)} className="bg-white rounded-xl shadow-sm border border-stone-200 flex flex-col text-left hover:shadow-md transition-all cursor-pointer group">
                                    <div className="p-6 flex-1">
                                        <div className="flex justify-between items-start w-full mb-4">
                                            <div className="w-10 h-10 bg-indigo-50 text-indigo-500 rounded-lg flex items-center justify-center"><FileText size={20} strokeWidth={1.5} /></div>
                                            <div className="px-2 py-1 bg-stone-100 rounded text-[10px] font-bold uppercase tracking-wider text-slate-500">{project.status}</div>
                                        </div>
                                        <h3 className="font-serif text-xl text-slate-900 mb-2 group-hover:text-indigo-600 transition-colors">{project.title}</h3>
                                        <div className="flex items-center justify-between text-xs text-slate-400 font-medium mt-4">
                                            <span className="flex items-center gap-1"><Target size={12} /> {project.targetWordCount} words</span>
